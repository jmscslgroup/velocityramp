...
void velocityramp_step(void)
{
  ...
  // Chart: '<Root>/Chart'
  if (velocityramp_DW.temporalCounter_i1 < 255U) {
    velocityramp_DW.temporalCounter_i1 = static_cast<uint8_T>
  ...
  if (velocityramp_DW.is_active_c3_velocityramp == 0U) {
    ...
    if (velocityramp_B.In1.Linear.X > 15.0) {
      velocityramp_B.vel_setpoint = velocityramp_B.In1.Linear.X;
    } else {
      velocityramp_B.vel_setpoint = 15.0;
    }
  } else {
    switch (velocityramp_DW.is_c3_velocityramp) {
     ...
     case velocityramp_IN_HoldAtPeak:
      if (velocityramp_DW.temporalCounter_i1 >= 200U) {
        velocityramp_DW.is_c3_velocityramp = velocityramp_IN_DropBy10;
        velocityramp_B.vel_setpoint -= 5.0;
     ...
     case velocityramp_IN_HoldAtPeak1:
      if ((velocityramp_DW.temporalCounter_i1 >= 200U) || (fabs
           (velocityramp_B.In1.Linear.X - velocityramp_B.vel_setpoint) > 10.0))
      {
        velocityramp_DW.is_c3_velocityramp = velocityramp_IN_reset;
        if (velocityramp_B.In1.Linear.X > 15.0) {
          velocityramp_B.vel_setpoint = velocityramp_B.In1.Linear.X;
