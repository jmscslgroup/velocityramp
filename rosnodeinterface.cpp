...
// Generated 22-Jul-2021 18:07:50
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"velocityramp\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(velocityramp_M, (NULL));
        velocityramp_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(velocityramp_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(velocityramp_M)
                == (NULL)) || rtmGetStopRequested(velocityramp_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(velocityramp_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
